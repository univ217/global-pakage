...
@Slf4j
public class GlobalExceptionHandler { // 전역적으로 발생하는 예외를 잡아줘야 함.
    ...
    public ResponseEntity<ErrorResponse<?>> handleMethodArgumentNotValidException(MethodArgumentNotValidException e) {
        ...
        ErrorResponse<?> errorResponse = ErrorResponse.of(
                ErrorResponseCode.INVALID_HTTP_MESSAGE_BODY, e.getFieldError().getDefaultMessage()); // NotNull(message = "")
        return ResponseEntity.status(errorResponse.getHttpStatus()).body(errorResponse);
    }


    // requestbody로도 데이터를 받지만,
    ...
    // ex. 엔드포인트는 정확히 입력했지만 POST 요청을 GET 으로 보내는 경우
    ...
    // 비즈니스 로직 에러
    // ex. 이미 존재하는 회원인데 같은 이이디로 회원가입 시도
